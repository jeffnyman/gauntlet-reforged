...
is = {
  ...
  link: function (obj) {
    return is.element(obj) && obj.tagName == "A";
  },
};
  ...
  bool: function (obj, def) {
    if (is.valid(obj))
      return (
        ...
        obj.toString().toLowerCase() == "yes"
      );
    else return is.bool(def) ? def : false;
  },
  number: function (obj, def) {
    if (is.valid(obj)) {
      var x = parseFloat(obj);
      if (!isNaN(x)) return x;
    }
    return is.number(def) ? def : 0;
  },
  string: function (obj, def) {
    if (is.valid(obj)) return obj.toString();
    return is.string(def) ? def : "";
  },
};
...
Game = {
  ...
  Runner: Class.create({
    ...
    start: function() {
      ...
      var step = function() {
        Game.Gamepad.poll(); // no native "gamepad button changed" event exists, so poll once per frame
      ...
      (on.publish = function (event) {
        if (this.subscribers && this.subscribers[event]) {
          var subs = this.subscribers[event],
            ...
            n,
            max;
          for (n = 0, max = subs.length; n < max; n++) subs[n].apply(on, args);
        }
      }));
...
Game.Element = (function () {
  var query = function (selector, context) {
    if (is.array(context)) return Sizzle.matches(selector, context);
    else return Sizzle(selector, context);
  };

  var extend = function (obj) {
    if (is.array(obj)) {
      for (var n = 0, l = obj.length; n < l; n++) obj[n] = extend(obj[n]);
  ...
  var on = function (ele, type, fn, capture) {
    ele.addEventListener(type, fn, capture);
  };
  var un = function (ele, type, fn, capture) {
      ...
      if (attributes.hasOwnProperty(name) && is.valid(attributes[name])) {
        switch (name) {
          case "tag":
            break;
          case "html":
            ele.innerHTML = attributes[name];
            break;
          case "text":
            ele.appendChild(document.createTextNode(attributes[name]));
            break;
          case "dom":
            attributes[name] = is.array(attributes[name])
              ? attributes[name]
              : [attributes[name]];
  ...
  return {
    ...
    get: function (obj, context) {
      if (is.string(obj)) return extend(query("#" + obj, context)[0]);
      else if (is.element(obj) || is.window(obj) || is.document(obj))
        return extend(obj);
      else if (is.event(obj)) return extend(obj.target || obj.srcElement);
      else if (typeof obj == "object" && obj.tag) return extend(create(obj));
      else throw "not an appropriate type for DOM wrapping: " + ele;
    },
    ...
    instanceMethods: {
      ...
      on: function (type, fn, capture) {
        on(this, type, fn, capture);
        return this;
      },
      un: function (type, fn, capture) {
        un(this, type, fn, capture);
        return this;
      },
      ...
      relations: function (property, includeSelf) {
        var result = includeSelf ? [this] : [],
          ele = this;
        while ((ele = ele[property])) if (ele.nodeType == 1) result.push(ele);
      ...
      ancestors: function (includeSelf) {
        return this.relations("parentNode", includeSelf);
      },
      ...
      select: function (selector) {
        return Game.Element.all(selector, this);
      },
      down: function (selector) {
        return Game.Element.all(selector, this)[0];
      },
      up: function (selector, includeSelf) {
        return Game.Element.all(selector, this.ancestors(includeSelf))[0];
      },
      prev: function (selector) {
        return Game.Element.all(selector, this.previousSiblings())[0];
      },
      next: function (selector) {
        return Game.Element.all(selector, this.nextSiblings())[0];
      },
      ...
      removeChildren: function () {
        // NOTE: can't use :clear because its in DOM level-1 and IE bitches if we try to provide our own
      ...
      append: function (content) {
        if (is.string(content)) this.innerHTML += content;
        else if (is.array(content))
          for (var n = 0; n < content.length; n++) this.append(content[n]);
        else this.appendChild(Game.Element.get(content));
      },
      ...
      fadeout: function (options) {
        ...
        this.fading = Animator.apply(this, "opacity: 0", {
          ...
          onComplete: function () {
            this.hide();
            this.fading = null;
            this.style.opacity = is.ie
              ? 1
              : null; /* clear opacity after hiding, but IE doesn't allow clear so set to 1.0 for IE */
...
Game.Event = {
  ...
  mouseWheelDelta: function (ev) {
    if (is.valid(ev.wheelDelta)) return ev.wheelDelta / 120;
    else if (is.valid(ev.detail)) return -ev.detail / 3;
    else return 0;
  },
};
  ...
  match: function (map, keyCode, mode, context) {
    if (map.mode === mode) {
      if (
        !map.state ||
        !context ||
        map.state === context.current ||
        (is.array(map.state) && map.state.indexOf(context.current) >= 0)
      ) {
        if (
          map.key === keyCode ||
          (map.keys && map.keys.indexOf(keyCode) >= 0)
        ) {
          return true;
        }
      }
