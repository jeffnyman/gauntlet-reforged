Gauntlet = function() {
  ...
  var VERSION  = "1.0.0",
      ...
      TILE     = 32,
      STILE    = 32,
      VIEWPORT = { TW: 32, TH: 24 }, // widescreen play area. 32x24 is the widest that still fits
                                     // every map: the tightest is trainer5 at 32x24 tiles.
      CAMERA_PADDING_TILES = 4,    // extra breathing room (in tiles) kept around the players' bounding box
      CAMERA_SMOOTHING     = 0.05, // fraction of the distance-to-target zoom closed per frame
      ...
      MONSTER = {
        GHOST:  { sx: 0, sy: 4, frames: 3, fpf: FPS/10, score:  10, health:  4, speed: 140/FPS, damage: 100/FPS, selfharm: 30/FPS, canbeshot: true,  canbehit: false, invisibility: false,                     travelling: 0.5*FPS, thinking: 0.5*FPS, generator: { health:  8, speed: 2.5*FPS, max: 40, score: 100, sx: 32, sy: 4 }, smart: false, name: "ghost",  weapon: null                                                                                     },
        DEMON:  { sx: 0, sy: 5, frames: 3, fpf: FPS/10, score:  20, health:  4, speed:  80/FPS, damage:  60/FPS, selfharm: 0,      canbeshot: true,  canbehit: true,  invisibility: false,                     travelling: 0.5*FPS, thinking: 0.5*FPS, generator: { health: 16, speed: 3.0*FPS, max: 40, score: 200, sx: 32, sy: 5 }, smart: true, name: "demon",  weapon: { speed: 240/FPS, reload: 2*FPS, damage: 10, sx: 24, sy: 5, fpf: FPS/10, monster: true } },
        GRUNT:  { sx: 0, sy: 6, frames: 3, fpf: FPS/10, score:  30, health:  8, speed: 120/FPS, damage:  60/FPS, selfharm: 0,      canbeshot: true,  canbehit: true,  invisibility: false,                     travelling: 0.5*FPS, thinking: 0.5*FPS, generator: { health: 16, speed: 3.5*FPS, max: 40, score: 300, sx: 32, sy: 6 }, smart: true, name: "grunt",  weapon: null                                                                                     },
        WIZARD: { sx: 0, sy: 7, frames: 3, fpf: FPS/10, score:  30, health:  8, speed: 120/FPS, damage:  60/FPS, selfharm: 0,      canbeshot: true,  canbehit: true,  invisibility: { on: 3*FPS, off: 6*FPS }, travelling: 0.5*FPS, thinking: 0.5*FPS, generator: { health: 24, speed: 4.0*FPS, max: 20, score: 400, sx: 32, sy: 8 }, smart: true, name: "wizard", weapon: null                                                                                     },
        DEATH:  { sx: 0, sy: 8, frames: 3, fpf: FPS/10, score: 500, health: 12, speed: 180/FPS, damage: 120/FPS, selfharm: 6/FPS,  canbeshot: false, canbehit: false, invisibility: false,                     travelling: 0.5*FPS, thinking: 0.5*FPS, generator: { health: 16, speed: 5.0*FPS, max: 10, score: 500, sx: 32, sy: 9 }, smart: true, name: "death",  weapon: null                                                                                     }
      },
      TREASURE = {
      ...
      DEBUG = {
        ...
        NOGENERATORS: Game.qsBool("nogenerators"),
        NOTREASURE:   Game.qsBool("notreasure"),
  ...
  //===========================================================================
  // MONSTER PATHFINDING (hard mode)
  //===========================================================================

  // Neighbour offsets for the flow field. Orthogonals come first so a diagonal is
  // only ever taken when both orthogonals it squeezes between are clear - without
  // that check monsters would path diagonally through a wall corner they cannot
  // actually walk through, and then grind against it.
  var FLOW_UNREACHABLE = 0x7FFFFFFF,
      FLOW_REFRESH     = 6, // frames between rebuilds - 10x/sec at 60fps
      FLOW_DX  = [  0,  0, -1,  1, -1,  1, -1,  1 ],
      FLOW_DY  = [ -1,  1,  0,  0, -1, -1,  1,  1 ],
      FLOW_DIR = [ DIR.UP,     DIR.DOWN,     DIR.LEFT,      DIR.RIGHT,
  ...
  var cfg = {
    ...
    keys: [
      // player 1 (keyboard): join/select with 1-4, move with arrows, SPACE to fire, ENTER to nuke
      { key: Game.Key.ONE,    mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.WARRIOR,  this.keyboardController('p1')); } },
      { key: Game.Key.TWO,    mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.VALKYRIE, this.keyboardController('p1')); } },
      { key: Game.Key.THREE,  mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.WIZARD,   this.keyboardController('p1')); } },
      { key: Game.Key.FOUR,   mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.ELF,      this.keyboardController('p1')); } },
      { key: Game.Key.LEFT,   mode: 'down', state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveLeft(true);   } },
      { key: Game.Key.RIGHT,  mode: 'down', state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveRight(true);  } },
      { key: Game.Key.UP,     mode: 'down', state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveUp(true);     } },
      { key: Game.Key.DOWN,   mode: 'down', state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveDown(true);   } },
      { key: Game.Key.LEFT,   mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveLeft(false);  } },
      { key: Game.Key.RIGHT,  mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveRight(false); } },
      { key: Game.Key.UP,     mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveUp(false);    } },
      { key: Game.Key.DOWN,   mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.moveDown(false);  } },
      { key: Game.Key.SPACE,  mode: 'down', state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.fire(true);       } },
      { key: Game.Key.SPACE,  mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.fire(false);      } },
      { key: Game.Key.RETURN, mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p1Keyboard; if (p) p.nuke();           } },

      // player 2 (keyboard): join/select with 5-8, move with WASD, F to fire, G to nuke
      { key: Game.Key.FIVE,   mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.WARRIOR,  this.keyboardController('p2')); } },
      { key: Game.Key.SIX,    mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.VALKYRIE, this.keyboardController('p2')); } },
      { key: Game.Key.SEVEN,  mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.WIZARD,   this.keyboardController('p2')); } },
      { key: Game.Key.EIGHT,  mode: 'up',   state: ['menu', 'playing'], action: function() { this.pressStart(PLAYER.ELF,      this.keyboardController('p2')); } },
      { key: Game.Key.A,      mode: 'down', state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveLeft(true);   } },
      { key: Game.Key.D,      mode: 'down', state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveRight(true);  } },
      { key: Game.Key.W,      mode: 'down', state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveUp(true);     } },
      { key: Game.Key.S,      mode: 'down', state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveDown(true);   } },
      { key: Game.Key.A,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveLeft(false);  } },
      { key: Game.Key.D,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveRight(false); } },
      { key: Game.Key.W,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveUp(false);    } },
      { key: Game.Key.S,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.moveDown(false);  } },
      { key: Game.Key.F,      mode: 'down', state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.fire(true);       } },
      { key: Game.Key.F,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.fire(false);      } },
      { key: Game.Key.G,      mode: 'up',   state: 'playing', action: function() { var p = this.controllers.p2Keyboard; if (p) p.nuke();           } },
    ...
    gamepad: [
      { button: Game.Gamepad.BUTTON.A,     mode: 'up',   state: ['menu', 'playing'], action: function(i) { this.pressStart(PLAYER.WARRIOR,  { kind: 'gamepad', index: i }); } },
      { button: Game.Gamepad.BUTTON.B,     mode: 'up',   state: ['menu', 'playing'], action: function(i) { this.pressStart(PLAYER.VALKYRIE, { kind: 'gamepad', index: i }); } },
      { button: Game.Gamepad.BUTTON.X,     mode: 'up',   state: ['menu', 'playing'], action: function(i) { this.pressStart(PLAYER.WIZARD,   { kind: 'gamepad', index: i }); } },
      { button: Game.Gamepad.BUTTON.Y,     mode: 'up',   state: ['menu', 'playing'], action: function(i) { this.pressStart(PLAYER.ELF,      { kind: 'gamepad', index: i }); } },
      { button: Game.Gamepad.BUTTON.RB,    mode: 'down', state: 'playing', action: function(i) { var p = this.controllers.gamepads[i]; if (p) p.fire(true);  } },
      { button: Game.Gamepad.BUTTON.RB,    mode: 'up',   state: 'playing', action: function(i) { var p = this.controllers.gamepads[i]; if (p) p.fire(false); } },
      { button: Game.Gamepad.BUTTON.LB,    mode: 'down', state: 'playing', action: function(i) { var p = this.controllers.gamepads[i]; if (p) p.nuke();      } },
      { button: Game.Gamepad.BUTTON.START, mode: 'up',   state: 'playing', action: function()  { this.quit();   } },
      { button: Game.Gamepad.BUTTON.START, mode: 'up',   state: 'help',    action: function()  { this.resume(); } }
    ]
  ...
  // picks a target for monster AI out of multiple players: nearest ALIVE player by preference,
  // falling back to the nearest player overall if everyone happens to be dead (still gives
  // monsters something to reference rather than nothing). reuses the same rough-but-fast
  // Chebyshev distance approximation Map.nuke already uses instead of a real sqrt(dx*dx+dy*dy)
  function nearestPlayer(entity, players) {
    var best = null, bestDist = Infinity, n, max, p, d;
    for (n = 0, max = players.length ; n < max ; n++) {
      p = players[n];
      if (p.dead) continue;
      d = Math.max(Math.abs(entity.x - p.x), Math.abs(entity.y - p.y));
  ...
  // shared-camera helpers: bound/center on the whole group of joined players (dead or alive —
  // corpses stay on screen too) rather than a single one
  function boundingBox(players) {
    var minx = Infinity, maxx = -Infinity, miny = Infinity, maxy = -Infinity, n, max, p;
    for (n = 0, max = players.length ; n < max ; n++) {
      p = players[n];
  ...
  function boundingCenter(players) {
    var box = boundingBox(players);
  ...
  var game = {
    ...
    run: function(runner) {
      ...
      Game.Gamepad.map(cfg.gamepad, this);

      // Browsers block audio.play() until the page has seen a real user gesture (click/key/touch),
      // so the automatic onmenu() -> playMenuMusic() call can get silently blocked if it fires first.
      // Retry once music is actually allowed, on the first genuine gesture of any kind.
      ...
      // clicking the curtain is itself the gesture the browser needs, so it just fades away;
      // unlockAudio above (bound on document, so it fires for this click too) handles starting the music
      $('curtain').on('click', function() {
        $('curtain').fadeout({ duration: 400 });
      });

      // "start from level 1" link (menu only, see CSS) — clears the saved level checkpoint so the
      // next game starts fresh from Training One instead of resuming where a previous session left off
      $('restart').on('click', function() {
        this.resetLevel();
      }.bind(this));

      // difficulty toggle (menu only, see CSS) - overlaid on the play area rather
      // than added to the scoreboard, which has no spare vertical room
      $('difficulty').on('click', function() {
        this.toggleHard();
      }.bind(this));

      // secret cheat: the classic Konami sequence toggles invulnerability, reusing the
      // DEBUG.NODAMAGE / DEBUG.NOAUTOHURT switches that already exist for ?nodamage=1 testing.
      // No prompt, no announcement — just a quiet glow (the same one heal() uses) to confirm it landed.
      var CHEAT = [Game.Key.UP, Game.Key.UP, Game.Key.DOWN, Game.Key.DOWN, Game.Key.LEFT, Game.Key.RIGHT, Game.Key.LEFT, Game.Key.RIGHT, Game.Key.B, Game.Key.A],
          progress = 0;
      document.addEventListener('keydown', function(ev) {
        progress = (ev.keyCode === CHEAT[progress]) ? progress + 1 : (ev.keyCode === CHEAT[0] ? 1 : 0);
        if (progress === CHEAT.length) {
          progress = 0;
          DEBUG.NODAMAGE = DEBUG.NOAUTOHURT = !DEBUG.NODAMAGE;
          if (this.players)
            for (var n = 0, max = this.players.length ; n < max ; n++)
              if (this.players[n].active())
                this.players[n].healing = FX.PLAYER_GLOW.frames;
        }
      }.bind(this), true);
      ...
      Game.loadResources(cfg.images, cfg.sounds, function(resources) {
        ...
        this.hard        = Game.qsBool("hard") || (this.storage[STORAGE.HARD] === "true");
        this.images      = resources.images;
        this.players     = [];
        this.controllers = { p1Keyboard: null, p2Keyboard: null, gamepads: {} };
        PLAYERS.forEach(function(type) { type.instance = new Player(); });
    ...
    onready: function() {
      ...
      if (Game.Math.between(DEBUG.LEVEL, 0, cfg.levels.length-1))
        this.start(PLAYER[DEBUG.PLAYER], DEBUG.LEVEL, { kind: 'keyboard', slot: 'p1' });
    },
    ...
    onstart: function(event, previous, current, type, nlevel, controller) {
      this.joinPlayer(type, controller);
    ...
    // single entry point for both keyboard and gamepad "join" bindings: starts a brand new game
    // from the menu (existing FSM path), or joins a new local player mid-run ("join-in-progress")
    // The number keys pick a CHARACTER; which keyboard set comes with it is decided
    // here. 1-4 prefer p1 (arrows) and 5-8 prefer p2 (WASD), but either falls back to
    // the other set when its preferred one is already claimed. Without the fallback,
    // 2/3/4 go dead the moment anyone joins - the scoreboard still invites you to press
    // them, but only 5-8 would actually add a second player.
    keyboardController: function(preferred) {
      var other = (preferred === 'p1') ? 'p2' : 'p1';
      if (!this.controllers[preferred + 'Keyboard']) return { kind: 'keyboard', slot: preferred };
      if (!this.controllers[other     + 'Keyboard']) return { kind: 'keyboard', slot: other     };
      return null; // both keyboard sets are in use
    },

    pressStart: function(type, controller) {
      if (type.instance.joined)          return; // this class is already taken
      if (!controller)                   return; // no free controller left for this input
      if (this.controllerTaken(controller)) return; // this physical device already controls a player
      if (this.is('menu'))
        this.start(type, undefined, controller);
      else if (this.is('playing'))
        this.joinPlayer(type, controller);
    },

    // claims a class, wires up its controller, and adds it to the active player roster; called
    // both for the very first player (via onstart) and for join-in-progress (via pressStart)
    joinPlayer: function(type, controller) {
      var player = type.instance;
      player.joined     = true;
      player.controller = controller;
      this.assignController(controller, player);
      player.join(type); // resets stats, publishes PLAYER_JOIN (Scoreboard reacts)
      set_add(this.players, player);
      if (this.map && this.is('playing')) // mid-run join: level's already loaded, no onStartLevel coming
        player.spawnNear(this.map, this.players.length > 1 ? this.players[0] : this.map.start);
      return player;
    },

    removePlayer: function(player) {
      set_remove(this.players, player);
      this.releaseController(player);
      player.joined = false;
      player.leave(); // publishes PLAYER_LEAVE (Scoreboard reacts)
    },

    controllerTaken: function(controller) {
      if (!controller) return false;
      return controller.kind === 'gamepad'
        ? !!this.controllers.gamepads[controller.index]
        : !!this.controllers[controller.slot + 'Keyboard'];
    },

    assignController: function(controller, player) {
      if (!controller) return;
      if (controller.kind === 'gamepad')
        this.controllers.gamepads[controller.index] = player;
      else
        this.controllers[controller.slot + 'Keyboard'] = player;
    },

    releaseController: function(player) {
      if (!player.controller) return;
      if (player.controller.kind === 'gamepad')
        delete this.controllers.gamepads[player.controller.index];
      else
        this.controllers[player.controller.slot + 'Keyboard'] = null;
    ...
    update: function(frame) {
      if (this.canUpdate) {
        for (var n = 0, max = this.players.length ; n < max ; n++)
          this.players[n].update(frame, this.players, this.map, this.viewport);
    ...
    draw: function(ctx, frame) {
      if (this.canDraw) {
        ...
        for (var n = 0, max = this.players.length ; n < max ; n++) {
          this.render.player(ctx, frame, this.viewport, this.players[n]);
          this.scoreboard.refreshPlayer(this.players[n]);
        }
      }
    ...
    onPlayerDeath: function() {
      if (this.players.length && this.players.every(function(p) { return p.dead; }))
        this.lose();
    },
    ...
    // fires once per player, as each begins their own exit animation — awards score immediately,
    // but the win/advance decision waits for onPlayerExit below, since deciding here would fire
    // as soon as the FIRST of several players starts exiting, stranding anyone still playing
    onPlayerExiting: function(player, exit) {
      player.addscore(this.map.level.score);
    },

    // fires once per player when THEIR exit animation fully completes. advance/win only once
    // every joined player has either exited or died — dead players must count as "done", or a
    // level with one dead player and one exited player would softlock forever (a dead player can
    // never publish PLAYER_EXIT)
    onPlayerExit: function(player) {
      player.exited = true;
      if (this.players.every(function(p) { return p.dead || p.exited; })) {
        if (this.map.last)
          this.win();
        else
          this.nextLevel();
    ...
    onPlayerCollide: function(player, entity) {
      if (entity.monster || entity.generator)
        ...
        player.keys--;
      else if (entity.exit === true) // strict check: Player also has an `exit` property (its own exit-animation
        player.exit(entity);         // method), which is truthy too and would otherwise misfire on player-vs-player contact
    },
    ...
    // clears just the saved level checkpoint (keeps high score intact) so the next game starts
    // fresh from Training One instead of resuming — wired to the "start from level 1" menu link
    resetLevel: function() {
      delete this.storage[STORAGE.NLEVEL];
      this.scoreboard.refreshLevel(cfg.levels[this.loadLevel()]);
    },
    ...
    // hard mode: monsters (bar ghosts) path around walls instead of beelining.
    // ?hard in the url forces it on for a session; the menu toggle persists it.
    toggleHard: function() {
      this.hard = !this.hard;
      this.storage[STORAGE.HARD] = this.hard ? "true" : "false";
      if (this.map)
        this.map.flowFrame = -FLOW_REFRESH; // rebuild the field on the next tick
      this.refreshDifficulty();
    },

    refreshDifficulty: function() {
      $('difficulty').update('Difficulty: ' + (this.hard ? 'Hard' : 'Normal'))
                     .setClassName(this.hard ? 'hard' : 'normal');
    },
    ...
    saveHighScore: function() {
      var best = null;
      for (var n = 0, max = this.players.length ; n < max ; n++)
        if (!best || this.players[n].score > best.score)
          best = this.players[n];
      if (best && best.score > this.loadHighScore()) {
        this.storage[STORAGE.SCORE] = best.score;
        this.storage[STORAGE.WHO]   = best.type.name;
      }
    },
  ...
  var Map = Class.create({
    ...
    occupied: function(x, y, w, h, ignore) {
      ...
      // have to check for any player FIRST, so even if a player is near a wall or other monster they will still get hit (otherwise its possible to use monsters as semi-shields against other monsters)
      for (var pn = 0, pmax = game.players.length ; pn < pmax ; pn++) {
        var p = game.players[pn];
        if ((p != ignore) && overlapEntity(x, y, w, h, p))
          return p;
      }
    ...
    //-------------------------------------------------------------------------
    // FLOW FIELD - shared monster pathing for hard mode
    //
    // A multi-source breadth-first search out from every live player, recording
    // how many steps each walkable tile is from the nearest one. A monster then
    // just steps to whichever neighbouring tile holds the smaller number, which
    // walks it around walls and through doorways instead of grinding into the
    // corner nearest the player.
    //
    // The point of a shared field is cost: this is one sweep of the grid for the
    // whole map (9,216 tiles on the largest level) rather than a search per
    // monster, and each monster's decision drops to a scan of 8 neighbours.
    //-------------------------------------------------------------------------

    updateFlow: function(frame, players) {
      if (!game.hard)
        return; // normal difficulty never reads the field, so never pay for it
      if (!this.flow)
        this.initFlow();
      if ((frame - this.flowFrame) < FLOW_REFRESH)
        return;
      this.flowFrame = frame;
      this.refreshFlow(players);
    },

    initFlow: function() {
      var n = this.tw * this.th, i, cell;
      this.flow      = new Int32Array(n);
      this.flowWall  = new Uint8Array(n); // static: walls never move
      this.flowBlock = new Uint8Array(n); // per refresh: walls + still-closed doors
      this.flowQueue = new Int32Array(n); // each tile is enqueued at most once
      this.flowFrame = -FLOW_REFRESH;
      for (i = 0 ; i < n ; i++) {
        cell = this.cells[i];
        this.flowWall[i] = (!cell || cell.wall !== undefined) ? 1 : 0;
      }
    },

    refreshFlow: function(players) {

      var tw = this.tw, th = this.th, n = tw * th,
          flow = this.flow, blocked = this.flowBlock, queue = this.flowQueue,
          head = 0, tail = 0,
          i, e, ti, tx, ty, d, nx, ny, ni;

      blocked.set(this.flowWall);

      // monsters cannot open doors, so a door that is still standing is a wall as
      // far as they are concerned. map.remove() clears .active when one opens.
      for (i = 0 ; i < this.entities.length ; i++) {
        e = this.entities[i];
        if (e.door && e.active) {
          ti = p2t(e.x) + (p2t(e.y) * tw);
          if (ti >= 0 && ti < n)
            blocked[ti] = 1;
        }
      }
      ...
      for (i = 0 ; i < n ; i++)
        flow[i] = FLOW_UNREACHABLE;

      for (i = 0 ; i < players.length ; i++) {
        if (!players[i].active())
          continue; // dead players are not worth walking towards
        tx = p2t(players[i].x);
        ty = p2t(players[i].y);
        if (tx < 0 || ty < 0 || tx >= tw || ty >= th)
          continue;
        ti = tx + (ty * tw);
        if (!blocked[ti] && flow[ti] === FLOW_UNREACHABLE) {
          flow[ti]     = 0;
          queue[tail++] = ti;
        }
      }
      ...
      while (head < tail) {
        ...
        for (i = 0 ; i < 8 ; i++) {
          ...
          if (nx < 0 || ny < 0 || nx >= tw || ny >= th)
            continue;
          ni = nx + (ny * tw);
          if (blocked[ni] || flow[ni] !== FLOW_UNREACHABLE)
            continue;
          // no cutting a diagonal between two blocked tiles
          if (i >= 4 && (blocked[nx + (ty * tw)] || blocked[tx + (ny * tw)]))
            continue;
          flow[ni]      = d;
          queue[tail++] = ni;
        }
    ...
    // One step along the shortest walkable route to the nearest player, or null
    // when there is no route (monster sealed off, every player dead) so the caller
    // can fall back to the direct bearing. Ties are broken at random so a crowd
    // fans out over equally good routes instead of forming a single file.
    flowDirection: function(entity) {

      if (!this.flow)
        return null;

      var tw = this.tw, th = this.th, flow = this.flow,
          tx = p2t(entity.x + entity.cbox.x + (entity.cbox.w / 2)),
          ty = p2t(entity.y + entity.cbox.y + (entity.cbox.h / 2)),
          here, best = FLOW_UNREACHABLE, dir = null, ties = 0,
          i, nx, ny, v;
      ...
      if ((here === FLOW_UNREACHABLE) || (here === 0))
        return null; // no route, or already on top of the player
      ...
      for (i = 0 ; i < 8 ; i++) {
        ...
        v = flow[nx + (ny * tw)];
        if (v >= here)
          continue; // only ever step somewhere strictly closer
        if (v < best) {
          best = v; dir = FLOW_DIR[i]; ties = 1;
        }
        else if (v === best) {
          ties++;
          if (Game.Math.randomInt(1, ties) === 1)
            dir = FLOW_DIR[i];
        }
      }
  ...
  var Monster = Class.create({
    ...
    update: function(frame, players, map, viewport) {
      ...
      var player = nearestPlayer(this, players);
      if (!player)
        return; // no players joined at all (shouldn't normally happen during 'playing')
      ...
      // hard mode: everything except the ghost consults the shared flow field, so
      // it heads along the shortest walkable route rather than straight at the
      // player. Only the BEARING changes - the fallback list, the firing check and
      // the collision handling below are all untouched.
      var smart = game.hard && this.type.smart && !away,
          bearing = smart ? map.flowDirection(this) : null;

      // 'travelling' exists to stop the greedy search oscillating when it gets
      // walled: it commits to a sideways direction blindly for up to a second.
      // A monster following a route has no such oscillation to damp, and the blind
      // commit would just throw the route away, so skip it while the field is
      // steering. Monsters with no route fall through to the old behaviour.
      if (this.travelling > 0 && bearing === null)
        return this.step(map, player, this.dir, speed, countdown(this.travelling), !away);

      // otherwise find a new direction
      var dirs, n, max;
      dirs = PREFERRED_DIRECTIONS[bearing === null ? this.directionTo(player, away) : bearing];
      for(n = 0, max = dirs.length ; n < max ; n++) {
        if (this.step(map, player, dirs[n], speed, (bearing !== null) || (n < 2) ? 0 : this.type.travelling * (n-2), !away))
          return;
      }
  ...
  var Player = Class.create({
    ...
    onStartLevel: function(map) {
      if (!this.joined) // this Player instance is a pre-created, unclaimed slot (see game.run) — leave it alone
        return;
    ...
    // used for join-in-progress: places a newly-joined player next to an existing one (or at the
    // map start, if they're first) instead of waiting for the next level's onStartLevel
    spawnNear: function(map, pos) {
      var offsets = [ {dx: TILE, dy: 0}, {dx: -TILE, dy: 0}, {dx: 0, dy: TILE}, {dx: 0, dy: -TILE} ],
          spawn, n, max;
      for (n = 0, max = offsets.length ; n < max ; n++) {
        spawn = { x: pos.x + offsets[n].dx, y: pos.y + offsets[n].dy };
        if (!map.occupied(spawn.x + this.cbox.x, spawn.y + this.cbox.y, this.cbox.w, this.cbox.h, this))
          break; // found a free spot (falls back to the last-tried, possibly-occupied spot otherwise)
      }
      map.occupy(spawn.x, spawn.y, this);
      this.keys    = DEBUG.KEYS || 0;
      this.dead    = false;
      this.exiting = false;
      this.exited  = false;
  ...
  var Viewport = Class.create({
    ...
    // shared camera: bounds/zooms to keep every joined player in view, the way the original
    // 4-player cabinet kept the whole party on one screen. with exactly one player the bounding
    // box has zero extent, so this collapses to the old single-player centering behavior exactly
    // (see boundingBox/boundingCenter above) — no regression for solo play.
    update: function(frame, players, map, viewport) {

      if (!players.length)
        return;

      if (this.zoomingout) { // win/lose fade — unchanged behavior, just centers on the group
        this.zoom(1/TILE, boundingCenter(players), map);
        return;
      }
      ...
      var box = boundingBox(players),
          neededTw = Math.max(
            VIEWPORT.TW,
            p2t(box.maxx - box.minx) + 2*CAMERA_PADDING_TILES,
            (p2t(box.maxy - box.miny) + 2*CAMERA_PADDING_TILES) * (VIEWPORT.TW/VIEWPORT.TH)
          ),
          next = this.tw + (neededTw - this.tw) * CAMERA_SMOOTHING;

      if (Math.abs(next - this.tw) > 0.05)
        this.size(next, map);

      this.center(boundingCenter(players), map);
    }
